...
if __name__ == '__main__':
    ...
    for i in range(len(Trackers_Name)):
        ...
        ## 1.Benchmark Dataset GT
        # video_paths = glob.glob(os.path.join('/Disk/Tiny_Object_Tracking/VL-TOT/VL-SOT230', '*'))
        video_paths = glob.glob(os.path.join('/Disk/Tiny_Object_Tracking/VL-TOT/VL-SOT270', '*'))
        video_num = len(video_paths)

        ## 2.Results: Test
        # output_dir = os.path.join('results/Baseline_Results/VL-SOT230', Tracker_Name)
        output_dir = os.path.join('results/Baseline_Results/VL-SOT270', Tracker_Name)
        # Calculate Accuracy
        main(Tracker_Name, video_paths, video_num, output_dir, visulization=False)
