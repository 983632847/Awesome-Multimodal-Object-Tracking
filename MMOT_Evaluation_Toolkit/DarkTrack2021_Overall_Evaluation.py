from __future__ import absolute_import
from got10k.experiments import *
from siamfc import TrackerSiamFC

if __name__ == '__main__':
    ## Setup tracker
    # net_path = 'pretrained/siamfc/model.pth'
    # tracker = TrackerSiamFC(net_path=net_path)

    ## All Sequences
    att_names = ["All"]


    ## Scenarios
    sce_names = ['DarkTrack2021']

    for att_name in att_names:
        ## Setup experiments
        experiments = [
            ## Overall Performance.
            ## The sce_name is not used in this evaluation, you can keep the default setting (sce_names[0]).
            ExperimentDarkTrack2021('/Disk/Night_UAV_Tracking/Datasets/DarkTrack2021', att_name, sce_names[0]),
        ]

        ## Run tracking experiments (optional) and report performance
        for e in experiments:
            ## demo of SiamFC
            # e.run(tracker, visualize=False)                # run tracker
            # e.report([tracker.name])                       # report results

            #################################################
            ## 1.Overall Performance
            # report result (one tracker)
            # e.report(["MambaTrack"])

            # report results (multiple trackers)
            e.report([
                "MambaTrack", "CiteTracker", "VLT_TT", "VLT_SCAR", "JointNLT",
                "STARK50","MixFormerV2", "HiFT", "DiMP18", "DiMP50",
                "SiamAPN++","SiamAPN++_SCT", "HiFT_SCT",  "PrDiMP50", "PrDiMP50_SCT",
                "SiamRPN_SCT", "SiamRPN"
                     ])
