...
class ExperimentMATrack(object):
    ...
    def __init__(self, root_dir, att_name, sce_name, return_meta=True, result_dir='results', report_dir='reports', report_scenario_dir='reports_scenario'):
        super(ExperimentMATrack, self).__init__()
        self.return_meta = return_meta
        self.dataset = UWVLT(root_dir, self.return_meta)

        #############################################################################
        # results root
        self.result_dir = os.path.join(result_dir, 'Baseline_Results', 'MATrack')

        # report root
        self.report_dir = os.path.join(report_dir, 'MATrack')
        ...
        self.att_dict = {
            ...
            'Fast_motion':1,
            'Scale_variation':2,
            'Aspect_ratio_variation':3,
            'Tiny_target':4,
            'Length_Short':5, 'Length_Medium':6, 'Length_Long':7,

            'Camera_motion':8, 'Invisibility':9, 'Rotation':10,
            'Deformation':11, 'Similar_distractors':12, 'Illumination_variation':13,
            'Motion_blur':14,
        ...
        for name in tracker_names:
            ...
            for s, (_, anno, meta) in enumerate(self.dataset):
                ...
                if att == "All":
                    ...
                    # 10+14+15+16+3=58
                    # 'Low_resolution', 'Fast_motion', 'Scale_variation', 'Aspect_ratio_variation', 'Tiny_target'
                    if att_dict[att] <= 4:
                    ...
                    # 'Camera_motion', 'Invisibility',
                    # 'Rotation', 'Deformation', 'Similar_distractors', 'Illumination_variation', 'Motion_blur',
                    elif att_dict[att]>=8 and att_dict[att]<=14:
                        Select_FLAG = (meta['att'][att_dict[att]-2] == True)  # 6-12

                    else:
                        assert (att_dict[att] > 13)
    ...
    def plot_curves(self, tracker_names, att_name):
        ...
        if att_name == "All":
            title_name_suffix = " on WebUOT-1M"
        else:
