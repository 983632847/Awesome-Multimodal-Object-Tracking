...
from ..datasets import UAVAntiUAV
...
class ExperimentUAVAntiUAV_Overall(object):
    r"""Experiment pipeline and evaluation toolkit for UAVAntiUAV dataset.
    Args:
        root_dir (string): Root directory of UAVAntiUAV dataset.
        att_name (string):
                Overall Performance is "All";
                Attribute-Based Performance are:
    ...
    def __init__(self, root_dir, att_name, sce_name, return_meta=True, result_dir='results', report_dir='reports', report_scenario_dir='reports_scenario'):
        super(ExperimentUAVAntiUAV_Overall, self).__init__()
        self.return_meta = return_meta
        self.dataset = UAVAntiUAV(root_dir, self.return_meta)
        ...
        # report root
        self.report_dir = os.path.join(report_dir, 'UAVAntiUAV')
        ...
        self.att_dict = {
            ...
            'Rotation': 5, 'Similar_Distractors': 6, 'Illumination_Variations': 7, 'Motion_Blur': 8, 'Partial_Target_Information': 9,
            'Small_Object': 10, 'Fast_Motion': 11, 'Scale_Variation': 12, 'Aspect_Ratio_Variation': 13,
            'Length_short': 14, 'Length_medium': 15, 'Length_long': 16
        }
...
#             'Camera_Motion': 0, 'Viewpoint_Changes': 1, 'Partial_Occlusion': 2, 'Full_Occlusion': 3, 'Out_of_View': 4,
                ...
                if (att == "All"):
                    ...
                    if att_dict[att] <= 13:                       ## First 14 Attributes
                        Select_FLAG = (meta['att'][att_dict[att]] == True)
                    elif att_dict[att] == 14:
                        Select_FLAG = (meta['att'][14] == 0)     ## Length_short
                    elif att_dict[att] == 15:
                        Select_FLAG = (meta['att'][14] == 1)     ## Length_meduim
                    elif att_dict[att] == 16:
                        Select_FLAG = (meta['att'][14] == 2)     ## Length_long
                    else:
                        assert (att_dict[att] > 16)
    ...
    def plot_curves(self, tracker_names, att_name):
        ...
        if att_name == "All":
            title_name_suffix = " on UAVAntiUAV"
        else:
        ...
        matplotlib.rcParams.update({'font.size': 6.5})
        ...
        # fig, ax = plt.subplots()
        fig, ax = plt.subplots(figsize=(10, 4.5), dpi=300)   # VL-SOT270
        ...
        # legend = ax.legend(lines, legends, loc='center left', ncol=2)
        ...
        # matplotlib.rcParams.update({'font.size': 12})
        # matplotlib.rcParams.update({'font.size': 8.0})         # Top 21 methods, font size 8.0
        # matplotlib.rcParams.update({'font.size': 5.32})        # Top 43 methods, font size 4.32
        ...
        # legend = ax.legend(lines, legends, loc='center right', ncol=2)
