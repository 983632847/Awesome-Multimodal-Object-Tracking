...
class ExperimentVLTOT270(object):
    r"""Experiment pipeline and evaluation toolkit for VL-TOT dataset.

    Args:
        root_dir (string): Root directory of VL-TOT dataset.
        att_name (string):
                Overall Performance is "All";
                Attribute-Based Performance are 'Camera_Motion', 'Viewpoint_Changes', 'Partial_Occlusion', 'Full_Occlusion', 'Out_of_View',
    ...
    def __init__(self, root_dir, att_name, sce_name, return_meta=True, result_dir='results', report_dir='reports', report_scenario_dir='reports_scenario'):
        super(ExperimentVLTOT270, self).__init__()
        self.return_meta = return_meta
        self.dataset = VLTOT(root_dir, self.return_meta)

        #############################################################################
        # results root
        # self.result_dir = os.path.join(result_dir, 'Baseline_Results', 'VL-TOT-Test')
        self.result_dir = os.path.join(result_dir)

        # report root
        self.report_dir = os.path.join(report_dir, 'VL-SOT')
        ...
        self.att_dict = {
            'Camera_Motion':0, 'Viewpoint_Changes':1, 'Partial_Occlusion':2, 'Full_Occlusion':3, 'Out_of_View':4,
            'Rotation':5, 'Deformation':6, 'Similar_Distractors':7, 'Illumination_Variations':8, 'Motion_Blur':9,
            'Natural_Object':10, 'Artificial_Object':11, 'Partial_Target_Information':12, 'Brightness_low':13, 'Brightness_medium':14,
            'Brightness_high':15, 'Fast_Motion':16, 'Scale_variation':17, 'Aspect_Ratio_Variation':18, 'Length_short':19,
            'Length_medium':20, 'Length_long':21}
    ...
    def report(self, tracker_names):
        ...
        for name in tracker_names:
            ...
            for s, (_, anno, meta) in enumerate(self.dataset):
                ...
                Select_FLAG = False
                if (att == "All") or (att == "VL-SOT270") or (att == "VL-SOT230"):
                    Select_FLAG = True
                else:
                    if att_dict[att] <= 9:                       ## First Nine Attributes
                        Select_FLAG = (meta['att'][att_dict[att]] == True)
                    elif att_dict[att] == 10:
                        Select_FLAG = (meta['att'][10] == 0)     ## Natural_Object
                    elif att_dict[att] == 11:
                        Select_FLAG = (meta['att'][10] == 1)     ## Artificial_Object
                    elif att_dict[att] == 12:
                        Select_FLAG = (meta['att'][11] == True)  ## Partial_Target_Information
                    elif att_dict[att] == 13:
                        Select_FLAG = (meta['att'][12] == 0)     ## Brightness_low
                    elif att_dict[att] == 14:
                        Select_FLAG = (meta['att'][12] == 1)     ## Brightness_meduim
                    elif att_dict[att] == 15:
                        Select_FLAG = (meta['att'][12] == 2)     ## Brightness_high
                    elif att_dict[att] == 16:
                        Select_FLAG = (meta['att'][13] == True)  ## Fast_Motion
                    elif att_dict[att] == 17:
                        Select_FLAG = (meta['att'][14] == True)  ## Scale_variation
                    elif att_dict[att] == 18:
                        Select_FLAG = (meta['att'][15] == True)  ## Aspect_Ratio_Variation
                    elif att_dict[att] == 19:
                        Select_FLAG = (meta['att'][16] == 0)     ## Length_short
                    elif att_dict[att] == 20:
                        Select_FLAG = (meta['att'][16] == 1)     ## Length_meduim
                    elif att_dict[att] == 21:
                        Select_FLAG = (meta['att'][16] == 2)     ## Length_long
                    else:
                        assert (att_dict[att] > 21)
                ...
                if Select_FLAG:
                    ...
                    if len(boxes) != len(anno):
                        print(record_file, "Length Error!", "result:", len(boxes), "anno:", len(anno))
                        # break
    ...
    def plot_curves(self, tracker_names, att_name):
        ...
        if att_name == "All":
            title_name_suffix = " on VL-SOT"
        elif att_name == "VL-SOT230":
            title_name_suffix = " on VL-SOT230"
        elif att_name == "VL-SOT270":
            title_name_suffix = " on VL-SOT270"
        else:
        ...
        # ####################################################
        # ShowNum = min(20, len(tracker_names))    # Show top 21 methods
        ...
        # fig, ax = plt.subplots()
        fig, ax = plt.subplots(figsize=(10, 4), dpi=300)   # VL-SOT270
        ...
        legend = ax.legend(lines, legends, loc='upper left', bbox_to_anchor=(1.02, 1), borderaxespad=0, ncol=2) # VL-SOT270
        ...
        ax.set(xlabel='Location error threshold',
               ylabel='Precision',
               xlim=(0, thr_ce.max()), ylim=(0, 0.2), # VL-SOT270
