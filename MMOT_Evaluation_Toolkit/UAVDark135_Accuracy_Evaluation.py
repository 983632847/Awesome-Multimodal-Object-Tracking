...
def main(Tracker_Name: object, video_paths: object, video_num: object, output_dir: object, visulization: object = False) -> object:
    # report performance
    overall_performance = []
    ...
    return Tracker_Name, np.mean(overall_performance)

if __name__ == '__main__':
    Trackers_Name = [
                "MambaTrack", "CiteTracker", "VLT_TT", "VLT_SCAR", "JointNLT",
                "STARK50", "MixFormerV2", "PrDiMP50", "DiMP50", "DiMP18",
                "HiFT", "SiamRPN", "SiamAPN++","SiamAPN++_SCT", "SiamRPN_SCT",
                "HiFT_SCT", "PrDiMP50_SCT"
                     ]
    ...
    for i in range(len(Trackers_Name)):
        ...
        # Benchmark Dataset GT
        video_paths = glob.glob(os.path.join('/Disk/Night_UAV_Tracking/Datasets/UAVDark135', '*'))
        video_num = len(video_paths)

        # Results: Test
        output_dir = os.path.join('results/Baseline_Results/UAVDark135', Tracker_Name)

        # Calculate Accuracy
        _, mACC = main(Tracker_Name, video_paths, video_num, output_dir, visulization=False)
        mACC_All.append(Tracker_Name+"\t"+str(mACC)+"\n")

    with open("mACC.txt", "w+") as f:
        f.writelines(mACC_All)
    print('OK!')
