...
class BatchedVideoDatapoint:
    ...
    expressions: Optional[torch.FloatTensor]
...
def collate_fn(
    ...
    for video_idx, video in enumerate(batch):
        ...
        for t, frame in enumerate(video.frames):
            ...
            for obj in objects:
                ...
                if obj.expression is not None and t == 0:
                    has_expressions = True
                    all_expressions.append(obj.expression)
    ...
    if not has_expressions:
        ...
        # B*N, C
        expressions = torch.stack(all_expressions, dim=0)
        expressions = expressions[None].expand(T, -1, -1).contiguous()
