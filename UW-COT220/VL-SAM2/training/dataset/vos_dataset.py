...
class VOSDataset(VisionDataset):
    ...
    def _get_datapoint(self, idx):
        ...
        datapoint = self.construct(
            video, sampled_frms_and_objs, segment_loader, expressions=expressions
        )
        for transform in self._transforms:
            datapoint = transform(datapoint, epoch=self.curr_epoch)
        return datapoint

    def construct(self, video, sampled_frms_and_objs, segment_loader, expressions=None):
        """
        ...
        for frame_idx, frame in enumerate(sampled_frames):
            ...
            for obj_id in sampled_object_ids:
                ...
                if expressions is not None:
                    if obj_id in expressions:
                        exp = expressions[obj_id]
                    else:
                        exp = "Object"

                    m = hashlib.sha256()
                    m.update(exp.encode("utf-8"))
                    exp_hash = m.hexdigest()

                    cache_path = f"/mnt/sdb/zhangchunhui/SAM2/VL-SAM2/cache/clip_lang/{exp_hash}.pt"
                    if os.path.exists(cache_path):
                        exp_embedding = torch.load(cache_path)
                    else:
                        raise ValueError("Expression not found in cache")
                else:
                    exp_embedding = None
