...
class PNGRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        video = VOSVideo(video_name, idx, frames, None)
...
class RefPNGRawDataset(VOSRawDataset):
    def __init__(
        self,
        root_dir,
        split,
        ...
        with open(os.path.join(root_dir, "meta_expressions", split, "meta_expressions.json"), "r") as f:
            self.annos = json.load(f)["videos"]

        # Check if it's not in excluded_files
        self.video_names = sorted(
            [key for key in self.annos.keys()]
        )
    ...
    def get_video(self, idx):
        ...
        video_frame_root = os.path.join(self.root_dir, self.split, "JPEGImages", video_name)

        video_mask_root = os.path.join(self.root_dir, self.split, "Annotations", video_name)
        ...
        for _, fpath in enumerate(all_frames[:: self.sample_rate]):
            ...
            frames.append(VOSFrame(fid, image_path=fpath))
        expressions = {
            int(exp["obj_id"]): exp["exp"]
            for exp in self.annos[video_name]["expressions"].values()
        }
        video = VOSVideo(video_name, idx, frames, expressions)
        return video, segment_loader
...
class SA1BRawDataset(VOSRawDataset):
    ...
    def get_video(self, idx):
        ...
        video = VOSVideo(video_name, int(video_name), frames, None)
        return video, segment_loader
...
class JSONRawDataset(VOSRawDataset):
    ...
    def get_video(self, video_idx):
        ...
        video = VOSVideo(video_name, video_idx, frames, None)
        return video, segment_loader
